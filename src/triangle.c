...
void draw_textured_triangle(int x0, int y0, int x1, int y1, int x2, int y2, color_t * texture) {
  if (y0 > y1) {
    swapi(&y0, &y1);
    swapi(&x0, &x1);
  }
  ...
  if (y1 == y2) {
    texture_flat_bottom_triangle(x0, y0, x1, y1, x2, y2, texture);
  } else if (y0 == y1) {
    texture_flat_top_triangle(x0, y0, x1, y1, x2, y2, texture);
  } else {
    int ym = y1;
    int xm = (((x2 - x0) * (y1 - y0)) / (y2 - y0)) + x0;

    texture_flat_bottom_triangle(x0, y0, x1, y1, xm, ym, texture);
    texture_flat_top_triangle(x1, y1, xm, ym, x2, y2, texture);
  }
}
